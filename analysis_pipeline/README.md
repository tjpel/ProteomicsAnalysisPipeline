...
- "delivery_format": Values: One of `["png", "jpg", "pdf"]`. When "write_fig_to_delivery" is set to "True", this configuration determines what file type the visualization will be stored as.
- "volcano": This object controls some behaviors and appearances of the volcano plot. Within the volcano plot, a point will be marked significant (red) if it both has a <i>P</i>-value below the P_value_threshold and a log<sub>2</sub>(Fold Change) above the Log2FC_threshold.
    ...
    - "annotations": Values: One of `["True", "False"]`. When set to "True", annotations of protein identifiers will be added to all significant proteins. This feature may be expanded soon to allow a limit of how many significant proteins receive annotations.
